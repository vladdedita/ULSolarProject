...
public interface MeasurementDao extends CrudRepository<Measurement, Long> {
    ...
    @Query(value = "SELECT s.id,s.power,s.time,s.direction,s.device_id FROM solar s WHERE TIMESTAMPDIFF(MINUTE,s.time,NOW()) < :time and s.device_id=:deviceId ORDER BY s.time ASC", nativeQuery = true)
